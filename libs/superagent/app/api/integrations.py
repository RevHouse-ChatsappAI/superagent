...
async def get_api_chatwoot_platform_extend(api_user=Depends(get_current_api_user)) -> ApiChatwootPlatformExtend:
    platform_key = await prisma.platformkey.find_first(
        where={'apiUserId': api_user.id}
    )

    if not platform_key:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No tienes key disponible")
    else:
        return ApiChatwootPlatformExtend(api_key=platform_key.key, base_url=platform_key.url)

@router.post("/integration/users", response_model=Any)
async def create_user(user_details: dict, api_chatwoot: ApiChatwootPlatformExtend = Depends(get_api_chatwoot_platform_extend)):
...
async def create_agent_bot(agent_bot_details: dict, api_chatwoot: ApiChatwootPlatformExtend = Depends(get_api_chatwoot_platform_extend)):
    try:
...
async def create_account_user(account_id: int, user_details: dict, api_chatwoot: ApiChatwootPlatformExtend = Depends(get_api_chatwoot_platform_extend)):
    try:
...
async def create_account(account_details: dict, api_chatwoot: ApiChatwootPlatformExtend = Depends(get_api_chatwoot_platform_extend)):
    try:
