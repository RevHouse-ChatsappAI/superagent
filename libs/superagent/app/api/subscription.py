...
async def create(body: ApiPaymentRequest, api_user=Depends(get_current_api_user)):
    ...
    tier_record = await prisma.tiercredit.find_unique(where={"tier": tier_name})
    if not tier_record:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    credit_data = {"apiUserId": api_user.id, "credits": credits, "subscriptionId": None}
...
async def get_payment(api_user=Depends(get_current_api_user)):
    ...
    payment = await prisma.subscription.find_first(where={"apiUserId": api_user.id})
    if not payment:
        response_data = {"success": False, "message": "Payment not found", "data": None}
    else:
        response_data = {
            "success": True,
...
async def get_count(api_user=Depends(get_current_api_user)):
    count = await prisma.count.find_first(where={"apiUserId": api_user.id})
    if not count:
        response_data = {"success": False, "message": "Credits not found", "data": None}
    else:
        response_data = {
...
async def post_free_account(
    body: ApiPaymentRequest, api_user=Depends(get_current_api_user)
):
