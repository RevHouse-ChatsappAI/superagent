...
export const SettingCRM = ({ profile, profileSAgent, platformKey}: Props) => {
  ...
  return (
    <form onSubmit={handleUpdateKeyPlatform} className="flex flex-1 flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        <Label>
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-gray-400">Introduce la clave de tu plataforma CRM Chatwoot para establecer la conexión y habilitar la integración con nuestro Super Agente.</p>
          </div>
        </Label>
        <Input onChange={e => setKey(e.target.value)} type="password" value={key ?? ''} />
      </div>
      <div className="flex flex-col gap-2">
        <Label>Nombre</Label>
      ...
      <div className="flex justify-end">
        <Button type="submit">{loading ? <Spinner/> : "Guardar cambios"}</Button>
      </div>
    </form>
  )
}
