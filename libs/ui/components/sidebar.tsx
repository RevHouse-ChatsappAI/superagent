...
export default function Sidebar() {
  ...
  return (
    <div
      className={`dark:bg-white-100 flex h-full w-[200px] flex-col justify-between space-y-3 rounded-r-2xl border-r bg-slate-50 py-4 align-top ${
        !session && "hidden"
      }`}
    >
      <div className="flex flex-col space-y-4">
        ...
        <div className="flex flex-col px-2">
          <p className="px-2 text-xs dark:text-gray-300">{workspaceNav.title}</p>
          {workspaceNav.items.map((navItem) => (
            <NextLink href={navItem.href} key={navItem.title}>
              <ButtonSidebar
                ...
                size="icon"
                className="flex w-full items-center justify-start bg-transparent px-3 dark:text-white"
              >
        ...
        <div>
          <p className="px-4 text-xs dark:text-gray-300">{knowledgeBaseNav.title}</p>
          <div className="flex flex-col px-2">
        ...
        <div>
          <p className="px-4 text-xs dark:text-gray-300">{apiBaseNav.title}</p>
          <div className="flex flex-col px-2">
      ...
      <div>
        <p className="px-4 text-xs dark:text-gray-300">{helpBaseNav.title}</p>
        <div className="flex flex-col px-2">
